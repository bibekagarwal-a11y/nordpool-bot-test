...
function parseContractStartMinutes(contractLabel) {
  if (!contractLabel || !contractLabel.includes("-")) {
    return null;
  }
...
function populateSelectors() {
  ...
  if (!areas.length) {
    throw new Error("No areas found in data.");
  }

  if (!rules.length) {
    throw new Error("No strategies found in data.");
  }
  ...
  setOptions("startDate", dates);
  setOptions("endDate", dates);
...
function renderMetricCards(filtered) {
  ...
  document.getElementById("profit").innerText = formatUnitValue(total);
  document.getElementById("contractCount").innerText = `${filtered.length}`;
  document.getElementById("avgProfit").innerText = formatUnitValue(avg);
  ...
  if (best) {
    bestEl.innerHTML = `
      <div class="positive-text metric-stack">
        <div>${best.date}</div>
        <div>${best.contract}</div>
        <div>${formatUnitValue(best.profit)}</div>
      </div>
    `;
  } else {
    bestEl.innerText = "-";
  ...
  if (worst) {
    worstEl.innerHTML = `
      <div class="negative-text metric-stack">
        <div>${worst.date}</div>
        <div>${worst.contract}</div>
        <div>${formatUnitValue(worst.profit)}</div>
      </div>
    `;
  } else {
    worstEl.innerText = "-";
...
function renderBessStrategy(filtered) {
  ...
  bessEl.innerHTML = `
    <strong>Charge:</strong> ${bestChargeRow.date} | ${bestChargeRow.contract} at ${formatUnitValue(bestChargeRow.buy_price)}
    <br>
    <strong>Discharge:</strong> ${bestDischargeRow.date} | ${bestDischargeRow.contract} at ${formatUnitValue(bestDischargeRow.sell_price)}
    <br>
    <strong>Single-cycle spread:</strong>
    <span class="${bestSpread >= 0 ? "positive-text" : "negative-text"}">${formatUnitValue(bestSpread)}</span>
  `;
}
...
function renderMultiCycleBess(filtered) {
  ...
  ordered.forEach((row, i) => {
    ...
    const shouldCharge =
      futureAvgSell !== null && soc < capacityMWh && buyPrice < chargeThreshold;
  ...
  el.innerHTML = `
    ...
    <span class="${totalPnL >= 0 ? "positive-text" : "negative-text"}">${totalPnL.toFixed(2)} EUR</span>
    <br>
    ...
    [
      {
        ...
        type: "histogram",
        marker: { color: "#2563eb" },
        hovertemplate: "Profit: %{x:.2f} EUR/MWh<br>Count: %{y}<extra></extra>",
      },
    ],
    {
      ...
      paper_bgcolor: "white",
      plot_bgcolor: "white",
      xaxis: { title: "Profit per row (EUR/MWh)", gridcolor: "#eaecf0" },
    ...
    [
      {
        ...
        type: "bar",
        marker: { color: colors },
        hovertemplate: "%{x}<br>Profit: %{y:.2f} EUR/MWh<extra></extra>",
      },
    ],
    {
      ...
      yaxis: { title: "Profit (EUR/MWh)", gridcolor: "#eaecf0" },
    },
    { responsive: true, displayModeBar: false }
  );
}
    ...
    [
      {
        ...
        marker: { size: 6 },
        hovertemplate: "%{x}<br>Cumulative: %{y:.2f} EUR/MWh<extra></extra>",
      },
    ],
    {
      margin: { l: 60, r: 20, t: 20, b: 120 },
      ...
      plot_bgcolor: "white",
      xaxis: { title: "Date | Contract", tickangle: -60, gridcolor: "#eaecf0" },
      yaxis: { title: "Cumulative P&L (EUR/MWh)", gridcolor: "#eaecf0" },
    },
    { responsive: true, displayModeBar: false }
  );
}
    ...
    [
      {
        ...
        reversescale: false,
        hovertemplate:
          "Date: %{x}<br>Contract: %{y}<br>Avg Profit: %{z:.2f} EUR/MWh<extra></extra>",
      },
    ],
    {
