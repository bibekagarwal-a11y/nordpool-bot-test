...
function showError(message) {
  ...
  if (table) {
    table.innerHTML = `<div class="error"><h2>Data loading error</h2><p>${message}</p></div>`;
  }
}
...
function renderMetricCards(filtered) {
  ...
  // we removed best/worst metrics for clarity
}
...
function renderBessStrategy(filtered) {
  ...
  bessEl.innerHTML = `Charge: ${bestChargeRow.date} | ${bestChargeRow.contract} at ${Number(bestChargeRow.buy_price).toFixed(2)} €/MWh<br>Discharge: ${bestDischargeRow.date} | ${bestDischargeRow.contract} at ${Number(bestDischargeRow.sell_price).toFixed(2)} €/MWh<br>Single-cycle spread: ${bestSpread.toFixed(2)} €/MWh`;
}
...
function renderMultiCycleBess(filtered) {
  ...
  el.innerHTML = `Estimated multi-cycle P&L: ${totalPnL.toFixed(2)} €<br>Charge actions: ${chargeActions}<br>Discharge actions: ${dischargeActions}<br>Total throughput: ${throughputMWh.toFixed(2)} MWh<br>Ending state of charge: ${soc.toFixed(2)} MWh`;
}
...
function buildMiniTable(rows) {
  return `<table class="mini-table"><thead><tr><th>Date</th><th>Contract</th><th>Buy</th><th>Sell</th><th>Profit</th></tr></thead><tbody>${rows
    .map(
      d =>
        `<tr><td>${d.date}</td><td>${d.contract}</td><td>${Number(d.buy_price).toFixed(2)}</td><td>${Number(d.sell_price).toFixed(2)}</td><td>${Number(d.profit).toFixed(2)}</td></tr>`
    )
    .join("")}</tbody></table>`;
}

function renderBreakdownTable(filtered) {
  document.getElementById("table").innerHTML = `<table><thead><tr><th>Date</th><th>Contract</th><th>Buy</th><th>Sell</th><th>Profit</th></tr></thead><tbody>${filtered
    .map(
      d =>
        `<tr><td>${d.date}</td><td>${d.contract}</td><td>${Number(d.buy_price).toFixed(2)}</td><td>${Number(d.sell_price).toFixed(2)}</td><td>${Number(d.profit).toFixed(2)}</td></tr>`
    )
